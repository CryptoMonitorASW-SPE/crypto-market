...
class ApiMetricsLoggingService(
    ...
    private val loggingIntervalMillis: Long = 60 * 1000L,
) {
    ...
    suspend fun startLogging() {
        ...
        while (isActive) {
            ...
            if (callsInInterval > 0) {
                logger.info(
                    "API Calls - Total: $totalCalls, Last ${loggingIntervalMillis / 1000} seconds: $callsInInterval at ${LocalDateTime.now()}",
                )
            }
        }
    }
